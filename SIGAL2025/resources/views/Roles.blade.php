...
@section('title', 'Roles - SIGAL')

@section('content')
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <h1 class="px-6 py-3">ROLES</h1>
    @if (session('success'))
    ...
    <div class="px-6 py-3 flex justify-end">
        <a href="{{ route('roles.crear') }}" class="inline-block px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300">Crear Rol</a>
    </div>
@endCanInsert
    </div>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">Descripción Rol</th>
                <th scope="col" class="px-6 py-3">Estado Rol</th>
        ...
        @if (!empty($roles) && count($roles) > 0)
            @foreach($roles as $rol)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="p-4">
                        <div class="flex items-center">
                            <input id="checkbox-{{ $rol['COD_ROL'] ?? 'undefined' }}" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            <label for="checkbox-{{ $rol['COD_ROL'] ?? 'undefined' }}" class="sr-only">Seleccionar</label>
                        </div>
                    </td>
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $rol['NOMBRE_ROL'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4">{{ $rol['DESCRIPCION_ROL'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4">{{ $rol['ESTADO_ROL'] == '1' ? 'Activo' : 'Inactivo' }}</td>
                    <td class="px-6 py-4">{{ $rol['USUARIO_CREA'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4">{{ $rol['FECHA_CREA'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4">
